...
# Импортируем классы из предыдущих шагов
from rag_pipeline.rag_metadata_questions_pipeline import MetadataQuestionsPipeline
from rag_pipeline.rag_question_analysis_pipeline import QuestionAnalysisPipeline
...
class RagFullPipeline:
    """
    Класс для запуска полного пайплайна RAG
    """
    def __init__(self):
        """
        Инициализация пайплайна с настройкой путей к файлам
        """
        # Пути для шага 1
        self.metadata_path = 'results/selected_metadata.json'
        self.questions_path = 'data/questions.json'
        self.companies_questions_path = 'results/companies_with_questions.json'
        
        # Пути для шага 2
        self.companies_questions_analyzed_path = 'results/all_questions_analysis_results.json'
        
        # Путь для финального результата
        self.final_results_path = 'results/final_results.json'
        
    def run_step1(self):
        """
        Запускает первый шаг пайплайна - связывание метаданных с вопросами
        """
        logger.info("Запуск шага 1: Связывание метаданных с вопросами")
        pipeline = MetadataQuestionsPipeline(
            self.metadata_path, 
            self.questions_path, 
            self.companies_questions_path
        )
        pipeline.run()
    ...
    def run_step2(self):
        """
        Запускает второй шаг пайплайна - анализ вопросов
        """
        logger.info("Запуск шага 2: Анализ вопросов")
        pipeline = QuestionAnalysisPipeline(
            self.companies_questions_path,
            self.companies_questions_analyzed_path
        )
        pipeline.run()
    ...
    def run_step3(self):
        """
        Запускает третий шаг пайплайна - обработка вопросов и генерация ответов
        """
        logger.info("Запуск шага 3: Обработка вопросов и генерация ответов")
        updated_companies = process_all_questions(self.companies_questions_analyzed_path)
        
        with open(self.final_results_path, 'w') as file:
            json.dump(updated_companies, file, indent=2, ensure_ascii=False)
        
        logger.info(f"Результаты сохранены в {self.final_results_path}")
        
    def run_full_pipeline(self):
        """
        Запускает все шаги пайплайна последовательно
        """
        logger.info("Запуск полного пайплайна RAG")
        ...
        logger.info("Полный пайплайн RAG успешно завершен")


if __name__ == "__main__":
    # Создаем и запускаем полный пайплайн
